...
// Create a website and add the admins and users,
// returns whether it was successful and a list of users it couldn't find
pub fn create_website<'a>(name: &'a str, conn: &PgConnection) -> Result<Website, Error> {
    use schema::websites;

    diesel::insert(&NewWebsite { name: name })
        .into(websites::table)
        .get_result(conn)
}
